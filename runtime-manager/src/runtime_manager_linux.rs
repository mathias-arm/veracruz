...
use crate::managers::{
    session_manager::{
        close_session, generate_csr, get_data, get_data_needed,
        init_session_manager, load_cert_chain, load_policy, new_session,
        send_data,
    },
    RuntimeManagerError,
};
        ...
        let return_message = match received_message {
            ...
            RuntimeManagerMessage::GetCSR => {
                ...
                let csr = generate_csr().map_err(|e| {
                    error!("Failed to generate certificate signing request.  Error produced: {:?}.", e);
