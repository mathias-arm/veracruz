...
pub mod veracruz_server_linux {
    ...
    impl VeracruzServer for VeracruzServerLinux {
            ...
            Self: Sized,
        {
            ...
            let csr = serialize(&RuntimeManagerMessage::GetCSR).map_err(|e| {
                error!("Failed to serialize CSR request message.  Error produced: {}.", e);

                VeracruzServerError::BincodeError(*e)
            })?;

            send_buffer(&mut runtime_manager_socket, &csr).map_err(|e| {
                error!("Failed to transmit CSR request message.  Error produced: {}.", e);

                e
            })?;

            let response = receive_buffer(&runtime_manager_socket).map_err(|e| {
                error!("Failed to receive response to CSR request.  Error produced: {}.", e);

                e
            })?;

            let csr_response = deserialize(&response).map_err(|e| {
                error!("Failed to deserialize response to CSR request.  Error produced: {}.", e);
            ...
            let csr = match csr_response {
                ...
                otherwise => {
                    error!("Received unexpected reponse from Linux runtime enclave: {:?}.", otherwise);
            ...
            let native_attestation = serialize(&LinuxRootEnclaveMessage::GetNativeAttestation).map_err(|e| {
                error!("Failed to serialize Native Attestation Token request.  Error produced: {}.", e);

                VeracruzServerError::BincodeError(*e)
            })?;

            send_buffer(&mut linux_root_socket, &native_attestation).map_err(|e| {
                error!("Failed to transmit Native Attestation Token request.  Error produced: {}.", e);

                e
            })?;

            let response = receive_buffer(&linux_root_socket).map_err(|e| {
                error!("Failed to received response to Native Attestation Token request.  Error produced: {}.", e);

                e
            })?;

            let native_attestation_response = deserialize(&response).map_err(|e| {
                error!("Failed to deserialize response to Native Attestation Token request.  Error produced: {}.", e);
            ...
            match native_attestation_response {
                ...
                otherwise => {
                    error!("Unexpected response from Linux Root Enclave.  Received: {:?}.", otherwise);
            ...
            let certificate_chain = serialize(&LinuxRootEnclaveMessage::GetProxyAttestation(csr, challenge_id)).map_err(|e| {
                error!("Failed to serialized request to obtain proxy attestation certificate chain.  Error produced: {}.", e);

                VeracruzServerError::BincodeError(*e)
            })?;

            send_buffer(&mut linux_root_socket, &certificate_chain).map_err(|e| {
                error!("Failed to transmit request to obtain proxy attestation certificate chain.  Error produced: {}.", e);

                VeracruzServerError::IOError(e)
            })?;

            let response_buffer = receive_buffer(&mut linux_root_socket).map_err(|e| {
                error!("Failed to receive response to request to obtain proxy attestation certificate chain.  Error produced: {}.", e);

                VeracruzServerError::IOError(e)
            })?;

            let response = deserialize(&response_buffer).map_err(|e| {
                error!("Failed to deserialize response to request to obtain proxy attestation certificate chain.  Error produced: {}.", e);
            ...
            let certificate_chain = serialize(&RuntimeManagerMessage::SetCertificateChain(
                certificate_chain,
            ))
            .map_err(|e| {
                error!(
                    "Failed to serialize certificate chain message.  Error produced: {}.",
                    e
                );
            ...
            send_buffer(&mut runtime_manager_socket, &certificate_chain).map_err(|e| {
                error!(
                    "Failed to transmit certificate chain message.  Error produced: {}.",
                    e
                );
            ...
            let response_buffer = receive_buffer(&runtime_manager_socket).map_err(|e| {
                error!(
                    "Failed to receive response to certificate chain message.  Error produced: {}.",
                    e
                );
            ...
            let response = deserialize(&response_buffer).map_err(|e| {
                error!("Failed to deserialize response to certificate chain message.  Error produced: {}.", e);
        ...
        fn tls_data(
            ...
            info!("Finished reading TLS data (active = {}, received {} bytes).", active, buffer.len());
