...
static RUNTIME_MANAGER_MEASUREMENT: [u8; 32] = [0u8; 32];

pub fn fake_attestation(challenge: &[u8], csr: Vec<u8>) -> Result<RuntimeManagerResponse> {
    ...
    let runtime_manager_hash: Vec<u8> = sha256(&RUNTIME_MANAGER_MEASUREMENT);
