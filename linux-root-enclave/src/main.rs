...
use veracruz_utils::{
    csr::{convert_csr_to_cert, generate_csr, COMPUTE_ENCLAVE_CERT_TEMPLATE, ROOT_ENCLAVE_CSR_TEMPLATE},
    platform::linux::{
...
fn post_buffer(url: &str, buffer: &str) -> Result<String, LinuxRootEnclaveError> {
    info!("Posting buffer {} ({} bytes) to {}.", buffer, buffer.len(), url);
        ...
        transfer
            .write_function(|buf| {
                received_body.push_str(from_utf8(buf).expect({
                    info!("Error converting data {:?} from UTF-8.  Continuing with default value.", buf);
...
fn post_native_attestation_token(
    ...
    let url = format!(
        "{}/PSA/AttestationToken", proxy_attestation_server_base_url,
    );
...
fn generate_device_private_key_pair(device_private_key: &[u8]) -> Result<EcdsaKeyPair, LinuxRootEnclaveError> {
    Ok(EcdsaKeyPair::from_pkcs8(&ECDSA_P256_SHA256_ASN1_SIGNING, device_private_key).map_err(|e| {
        error!("Failed to generate device private key pair.  Error produced: {:?}.", e);
...
fn native_attestation(
    ...
    *device_id_lock = Some(device_id.clone());
    ...
    let device_private_key_lock = DEVICE_PRIVATE_KEY.lock().map_err(|e| {
        error!("Failed to obtain lock on DEVICE_PRIVATE_KEY.  Error produced: {}.", e);
    ...
    let csr = generate_csr(&ROOT_ENCLAVE_CSR_TEMPLATE, &device_private_key_pair).map_err(|e| {
        error!("Failed to generate Root Enclave CSR.  Error produced: {:?}.", e);
    ...
    while !shutdown {
        ...
        let response = match received_message {
            ...
            LinuxRootEnclaveMessage::GetProxyAttestation(csr, challenge_id) => {
                info!("Computing a proxy attestation certificate chain (CSR {} bytes).", csr.len());
